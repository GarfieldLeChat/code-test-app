...
angular.module('app').controller('FilterController', function($scope, $http) {
  ...
  angular.element(document).ready(function() {
    ...
    if ($scope.records.drinks) {
      drinksSlider.setChecked(true);
    }
    if ($scope.records.arts) {
      artsSlider.setChecked(true);
    }
    ...
    drinksSlider.on('change', function(slider) {
      $scope.records.drinks = slider.value;
    });

    artsSlider.on('change', function(slider) {
      $scope.records.arts = slider.value;
    });
...
angular.module('app').controller('AppController', function($scope, $http) {
  ...
  $scope.filterClick = function() {
    navi.pushPage('templates/filter.html', {animation: 'lift' });
  };
  ...
  $scope.search = function() {
    ...
    if (records.drinks) category.push('drinks');
    if (records.arts) category.push('arts');

    var clientID = "Z3NRPLWAPE5GW4WXH3NDBKSDCV2D1Z5VIBTSWHP4SJEYQB4K";
    var clientSecret = "SMIA2E1UOJKZQBTOVNI1UI34GFK2P0UER3PAEZOE5ZNS2J3Y";
